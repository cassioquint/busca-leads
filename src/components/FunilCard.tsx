...
// 🔥 Definimos a interface simples de Bucket para o card conhecer a estrutura
interface Bucket {
  id: string;
  name: string;
}
...
interface FunilCardProps {
  lead: Lead;
  colId: string;
  buckets: Bucket[]; // 🔥 NOVO: Recebe a lista de colunas para calcular os limites dinamicamente
  onMoveLead: (id: string, direction: 'forward' | 'backward') => void;
}

export const FunilCard: React.FC<FunilCardProps> = ({ lead, colId, buckets = [], onMoveLead }) => {
  // Ajuste fino: pegamos o dia de hoje dinamicamente para o mock de urgência não quebrar no futuro
  const todayStr = new Date().toLocaleDateString('pt-BR');
  const isUrgent = lead.followUpDate === todayStr;

  // 🔥 LÓGICA DINÂMICA DE BLOQUEIO DAS SETAS
